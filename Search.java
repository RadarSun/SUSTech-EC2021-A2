...
public class Search {
	...
	////////////////////////////////////////////////////////////////////////
	////////////////////////////////////////////////////////////////////////
	public ArrayList<ArrayList<Item>> splitWithWeights(ArrayList<Item> product, int nL){
		System.out.println("******************************************************************************************");
		System.out.println("Using splitWithWeights");
		...
		if(product.size() <= 1){
			...
			System.out.println("All items:"+product.subList(0, product.size()));
			product = this.sortItemsInProduct(product);
			double avgofproduct = 0;
			avgofproduct = this.getAvgOfProduct(product);
			System.out.println("avagooproduct: "+ avgofproduct);

			double medianofproduct = 0;
			medianofproduct = this.getMedianOfProduct(product);
			System.out.println("medianofproduct: " + medianofproduct);

			// int point = getLocationInProduct(product, avgofproduct, low, high);
			// System.out.println("avgofproduct:"+avgofproduct+ "   point:"+point);

//			int point = getLocationInProduct(product, medianofproduct, low, high);
//			System.out.println("medianofproduct:"+medianofproduct+ "  point:"+point);
			int len = product.size();
			int[] data = new int[len];
			for(int i = 0;i<len;i++){
				data[i] = product.get(i).freq;
			}
			RunKMeans runKMeans1 = new RunKMeans(data);
			int point = runKMeans1.getPoint();
			System.out.println("8888888888888888888888888888:        "+point);

			res.add(convert(product.subList(0, point)));
			res.add(convert(product.subList(point, product.size())));
			// System.out.println("---------------");
			// System.out.println("before split: ");
			// System.out.println(product.subList(0, product.size()));
			// System.out.println("after split: ");
			System.out.println(product.subList(0, point));
			// System.out.println(sum(convert(product.subList(0, point))));
			System.out.println(product.subList(point,product.size()));
			// System.out.println(res);//res是分好的组
			// System.out.println("---------------");
	...
	public ArrayList<Item> sortItemsInProduct(ArrayList<Item> product){
		ArrayList<Item> sortedprodct = new ArrayList<Item>();
		sortedprodct = product;
		Collections.sort(sortedprodct, new Comparator<Item>(){
			@Override
			public int compare(Item l1, Item l2){
				return Integer.compare(-l1.freq, -l2.freq);//对所有的小gruop的freq和进行降序排列
			}
		});
		System.out.println("All items after sorting:" + sortedprodct);
	...
	////////////////////////////////////////////////////////////////////////
	////////////////////////////////////////////////////////////////////////
	public Double getAvgOfProduct(ArrayList<Item> product){
		double avgofproduct = 0;
		for(Item i :product){
			avgofproduct += i.freq;
		}
		return avgofproduct/product.size();
	}
	
	public Double getMedianOfProduct(ArrayList<Item> product){
		double medianofproduct = 0;
		// Collections.sort(product, new Comparator<Item>(){
		// 	@Override
		// 	public int compare(Item l1, Item l2){
		// 		return Integer.compare(-l1.freq, -l2.freq);//对所有的小gruop的freq和进行降序排列
		// 	}
		// });
		if(product.size()%2 == 0){
			medianofproduct = (product.get(product.size()/2-1).freq + product.get(product.size()/2).freq)/2;
		}
		else{
			medianofproduct = product.get(product.size()/2).freq;
		}
		return medianofproduct;
	}
	
	public int getLocationInProduct(ArrayList<Item> product,double num,int low,int high){
		int location = 0;
		int i = 0;
		// if(product.size() != 1){
			if(num == product.get(0).freq){//each item has the same fre, random split
				location = this.random.nextInt(high - low);
				location +=low;
			}
			else{
				for(i = 0; i<product.size();i++){
					if((num <= product.get(i).freq)&&(num >= product.get(i+1).freq)){
						location = i;
						break;
					} 
				}
	...
	
	public Layout creatLayout(SLAP slap, String splitMethod, boolean randomOrder){//randomOrder决定降序还是升序，true为升序，源代码为降序
		int ntries = 10;
		...
		for(int atmps = 0; atmps < ntries; atmps++){
			...
			// ArrayList<Double> freqOfProducts = new ArrayList<Double>();
			// double avarageFreq = 0;
			// for(ArrayList<Item> p : slap.product.values()){//p是每个product，里面包含了它的item
			// 	avarageFreq = sum(p)/p.size();
			// 	freqOfProducts.add(avarageFreq);
			// } 
			for(ArrayList<Item> p : slap.product.values()){//p是每个product，里面包含了它的item
				//int parameter
				System.out.println("--------------------------------------------------------------------------------------------------------------------------------------------");
				...
				try{
				    ...
				    Object objGrps =  method.invoke(this,p, slap.nL);//objGrps是一个product分好的组，groups把所以product的分组合起来
			...
			if(randomOrder){
				...
				Collections.sort(groups, new Comparator<ArrayList<Item>>(){
											...
											public int compare(ArrayList<Item> l1, ArrayList<Item> l2){
												return Integer.compare(-sum(l1), -sum(l2));//对所有的小gruop的freq和进行降序排列
											}
										});
			...
			for(ArrayList<Item> grp: groups){
				...
				for(int r: slap.S){
					 rowLength.add( Layout.countARow(layout.row.get(r)));//Layout.countARow(layout.row.get(r))得到的是一个row，即一个shelves上已经用掉的item数目,而rowLength包含所有shelves上用掉的item位置数目
				}
				
				if(slap.nL -  Collections.min(rowLength)  < grp.size()){//当前的小group没法放进位置最大的shelves，则退出for循环，输出警告：没有可行解
	                layout = null;
	                break; // give up
				}
				else
				{
					 // look for closest location where item will fit
		                ...
		                int ridx = 0;
		                for(int r: slap.S){//遍历所有的shelf
		                	if(rowLength.get(r) + grp.size() <= slap.nL){//加入这个小group不会超过这个shelf的容量
		                		if(rowLength.get(r) + r < cnt){//用来限制邻近的位置，用第一个shelf已装的大小+id 作为cnt的初始值，并用ridx记录最后满足条件的shelf id。每往后移动一个shelf，需要增加两个item的容量能力
		                			ridx = r;//3+1 = 4; 1+2<4;
		                			cnt = rowLength.get(r) + r;
		                		}
		                	}
		                }
		                layout.row.get(ridx).add(grp);//将当前的小group分配给最后满足条件的shelf
				}
